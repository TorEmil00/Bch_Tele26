...
#define RX_BUFFER_SAMPLES 8192  // Mindre buffer for lavere latency på RX
...
void *rx_thread_func(void *arg) {
    ...
    // --- DSP Oppsett ---
    // NYTT OG KORREKT: Innebygd DC-blokker fra liquid-dsp
    ...
    // Costas Loop / PLL for å låse fasen
    ...
    while (keep_running) {
        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            ...
            // Rens signalet for DC-støy (senterfrekvens) før volumjustering
            ...
            for (int i = 0; i < num_symbols; i++) {
                ...
                // Heartbeat for feilsøking (Satt til 20 kbps = 1 utskrift i sekundet)
                bit_count++;
                if (bit_count >= 20000) { 
    ...
    iirfilt_crcf_destroy(qdc); // Rydd opp DC-blokkeren
...
int main() {
    ...
    // 1. SET BANDWIDTH FIRST
    if (iio_channel_attr_write_longlong(tx_chan, "rf_bandwidth", 1000000) < 0) {
        fprintf(stderr, "Failed to set TX rf_bandwidth!\n");
    }

    // 2. SET SAMPLING FREQUENCY SECOND
    if (iio_channel_attr_write_longlong(tx_chan, "sampling_frequency", 1000000) < 0) {
        fprintf(stderr, "Failed to set TX sampling_frequency!\n");
    }
    ...
    // Note: voltage0 on phy_dev with false = RX channel
    ...
    // 1. SET BANDWIDTH FIRST
    // 200 kHz is the absolute lowest valid analog bandwidth for the AD9363.
    if (iio_channel_attr_write_longlong(rx_chan, "rf_bandwidth", 1000000) < 0) {
        fprintf(stderr, "Failed to set RX rf_bandwidth!\n");
    }

    // 2. SET SAMPLING FREQUENCY SECOND
    // Must be >= ~521 kHz and >= rf_bandwidth. Using 1 MHz to match your TX.
    if (iio_channel_attr_write_longlong(rx_chan, "sampling_frequency", 1000000) < 0) {
        fprintf(stderr, "Failed to set RX sampling_frequency!\n");
    }

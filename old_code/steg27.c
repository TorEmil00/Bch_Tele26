    ...
    while (keep_running) {
        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            if (bit_idx == 0 && sample_count == 0) {
                ...
                snprintf(payload, PAYLOAD_LEN, "ID:1|ALT:%.1f|SEQ:%d|ACK:%d|LCMD:%d|UTS:%.3f", 
                         rocket_alt, seq_num, cmd_ack_count, last_cmd_id, get_raw_time());
...
void *rx_thread_func(void *arg) {
    ...
    // NYTT: Bruker CPU-vennlige heltall for decimerings-loopen!
    int32_t decim_sum_i = 0;
    int32_t decim_sum_q = 0;
    char ts[20];
    ...
    while (keep_running) {
        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            // Lese rå-verdier direkte som heltall (Sparer CPU for divisjon)
            decim_sum_i += ((int16_t*)p)[0];
            decim_sum_q += ((int16_t*)p)[1];
            decim_count++;
            
            if (decim_count >= DECIMATION_FACTOR) {
                // Gjør flyttallsdivisjon KUN etter krympingen (90% mindre CPU bruk!)
                float real_part = (float)decim_sum_i / (2048.0f * DECIMATION_FACTOR);
                float imag_part = (float)decim_sum_q / (2048.0f * DECIMATION_FACTOR);
                decim_sum_i = 0; decim_sum_q = 0; decim_count = 0;

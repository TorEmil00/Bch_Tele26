        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            ...
            if (decim_count >= DECIMATION_FACTOR) {
                ...
                for (int i = 0; i < num_symbols; i++) {
                    ...
                    // FIKS: Rettet til 50 000 (Gir nøyaktig 1 utskrift i sekundet)
                    if (bit_count >= 50000) { 
                        printf("[RX Heartbeat] PLL Freq: %5.2f | Status: Venter på kommando...\n", nco_crcf_get_frequency(qpll));
                        fflush(stdout); // Tvinger terminalen til å printe umiddelbart!
                        bit_count = 0;
                    }
                    ...
                    if (shift_reg == SYNC_WORD || shift_reg == SYNC_INVERTED) {
                        ...
                        printf("=========================================\n\n");
                        fflush(stdout); // Tvinger BINGO til å smelle opp på skjermen med en gang!
                    }
                }
            }
        }

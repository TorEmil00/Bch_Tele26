...
// Hver symbol er 50 samples (2.5M / 50k), men på RX knuser vi 10 samples sammen til 1.
#define SAMPLES_PER_SYMBOL_TX 50  
#define SAMPLES_PER_SYMBOL_RX 5   
#define DECIMATION_FACTOR 10      // Gjør om 2.5 MHz til 250 kHz for DSP-en
...
void verify_radio_settings(struct iio_channel *tx_lo, struct iio_channel *tx_chan, 
    ...
    // Sjekker at radioen faktisk tok imot 2.5 MHz
    ...
    if (llabs(val - 2500000) <= 10) printf("[OK] RX Sample Rate: %lld Hz\n", val);
    ...
    if (llabs(val - 2500000) <= 10) printf("[OK] TX Sample Rate: %lld Hz\n", val);
...
void *tx_thread_func(void *arg) {
    ...
    while (keep_running) {
        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            ...
            sample_count++;
            if (sample_count >= SAMPLES_PER_SYMBOL_TX) { // Matchet til 2.5 MHz
...
void *rx_thread_func(void *arg) {
    ...
    while (keep_running) {
        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            ...
            // Bygger opp decimeringsbufferet
            decim_sum_i += i_val;
            decim_sum_q += q_val;
            decim_count++;

            // Kjører DSP bare på hver 10. sample (Sparer 90% CPU!)
            if (decim_count >= DECIMATION_FACTOR) {
                sample_in = (decim_sum_i * 0.1f) + _Complex_I * (decim_sum_q * 0.1f);
                ...
                for (int i = 0; i < num_symbols; i++) {
                    ...
                    bit_count++;
                    if (bit_count >= 50000) { // 50 kbps symbol rate = 1 melding i sekundet
...
int main() {
    ...
    // 2.5 MHz er en fysisk trygg sone for AD9361!

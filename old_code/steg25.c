...
// ==========================================
// TRÅD 1: SENDER TELEMETRI
// ==========================================
void *tx_thread_func(void *arg) {
    char ts[20];
    get_ts(ts, sizeof(ts));
    printf("[%s] [TX] Telemetri-tråd startet\n", ts);
    fflush(stdout);
...
// ==========================================
// TRÅD 2: MOTTAR KOMMANDOER
// ==========================================
void *rx_thread_func(void *arg) {
    char ts[20];
    get_ts(ts, sizeof(ts));
    printf("[%s] [RX] Kommando-tråd startet\n", ts);
    fflush(stdout);
    ...
    long bit_count = 0;
    int decim_count = 0;
    float decim_sum_i = 0.0f, decim_sum_q = 0.0f;
    ...
    while (keep_running) {
        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            decim_sum_i += ((int16_t*)p)[0] / 2048.0f;
            decim_sum_q += ((int16_t*)p)[1] / 2048.0f;
            decim_count++;
            ...
            if (decim_count >= DECIMATION_FACTOR) {
                ...
                for (int i = 0; i < num_symbols; i++) {
                    ...
                    if (bit_count >= 50000) { 
                        get_ts(ts, sizeof(ts));
                        printf("[%s] [RX Heartbeat] PLL Freq: %5.2f | Status: Lytter...\n", ts, nco_crcf_get_frequency(qpll));
                        fflush(stdout); 
                        bit_count = 0;
                    }
...
int main() {
    ...
    char ts[20];
    get_ts(ts, sizeof(ts));
    printf("[%s] 🚀 Rakett-OS Kjører! Klar for oppskytning.\n", ts);
    fflush(stdout);

...
// --- DEN DELTE POSTKASSEN ---
volatile int cmd_ack_count = 0; // Teller hvor mange kommandoer vi har fått!
...
// ==========================================
// TRÅD 1: SENDER TELEMETRI (Nå med ACK!)
// ==========================================
    ...
    while (keep_running) {
        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            ...
            if (bit_idx == 0 && sample_count == 0) {
                ...
                // NYTT: Vi sender nå med ACK-telleren i telemetrien!
                snprintf(payload, PAYLOAD_LEN, "ID:1 | ALT:%.1fm | SEQ:%d | ACK:%d", rocket_alt, seq_num, cmd_ack_count);
            ...
            if (decim_count >= DECIMATION_FACTOR) {
                ...
                for (int i = 0; i < num_symbols; i++) {
                    ...
                    if (shift_reg == SYNC_WORD || shift_reg == SYNC_INVERTED) {
                        // NYTT: Øker postkasse-telleren! TX-tråden vil fange opp dette umiddelbart.
                        cmd_ack_count++; 
                        ...
                        // Nullstiller registeret for ikke å telle samme pakke dobbelt
                        shift_reg = 0;
                    }
                }
            }

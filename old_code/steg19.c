...
// ==========================================
// TRÅD 1: SENDER TELEMETRI (Uendret støy for å holde link oppe)
// ==========================================
void *tx_thread_func(void *arg) {
    ...
    while (keep_running) {
        ...
        int bit_idx = 0; int sample_count = 0; int bit_val = rand() & 1;
        for (void *p = p_dat; p < p_end; p += p_inc) {
            ((int16_t*)p)[0] = bit_val ? AMPLITUDE : -AMPLITUDE; 
            ((int16_t*)p)[1] = 0;                                    
            sample_count++;
            if (sample_count >= SAMPLES_PER_SYMBOL_TX) { 
                sample_count = 0; bit_val = rand() & 1;
            }
        }
...
// ==========================================
// TRÅD 2: MOTTAR RAW BITS
// ==========================================
void *rx_thread_func(void *arg) {
    printf("[RX] Raw Mode Lytter på Kjerne 1\n");
        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            ...
            if (decim_count >= DECIMATION_FACTOR) {
                ...
                for (int i = 0; i < num_symbols; i++) {
                    ...
                    bit_count++;
                    if (bit_count >= 50000) { 
                        printf("[RX Heartbeat] PLL Freq: %5.2f | Siste bits i HEX: 0x%08X\n", nco_crcf_get_frequency(qpll), shift_reg);
                        bit_count = 0;
                    }

                    // RAW MODE SJEKK: Leter etter 11001100 (0xCCCCCCCC) eller invertert 00110011 (0x33333333)
                    if (shift_reg == 0xCCCCCCCC || shift_reg == 0x33333333) {
                        spam_filter++;
                        if (spam_filter > 10000) { // Forhindrer at terminalen krasjer av utskrift
                            printf("\n🟢 [SUKSESS] RAW BITS DETEKTERT! Linken er 100%% operativ!\n\n");
                            spam_filter = 0;
                        }
                    }
                }

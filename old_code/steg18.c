...
void *rx_thread_func(void *arg) {
    ...
    // Vi senker filterets båndbredde drastisk for å gjøre det robust mot støy
    ...
    while (keep_running) {
        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            ...
            if (decim_count >= DECIMATION_FACTOR) {
                // Gjennomsnitt etter desimering
                float real_part = decim_sum_i * 0.1f;
                float imag_part = decim_sum_q * 0.1f;
                decim_sum_i = 0.0f; decim_sum_q = 0.0f; decim_count = 0;

                // --- DEN MAGISKE KRASJ-BARRIEREN ---
                // Hvis amplituden skyter i været, klipper vi den av her før filteret ser den!
...
int main() {
    ...
    // --- LÅSER MASKINVARENS VOLUM (Hindrer smell ved Python-pauser) ---
    iio_channel_attr_write(rx_chan, "gain_control_mode", "manual");
    iio_channel_attr_write_longlong(rx_chan, "hardwaregain", 40); // 40 dB fast forsterkning

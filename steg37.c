...
volatile int uart_ack_count = 0; // NY: Teller vellykkede UART-utskrifter
...
void apply_cmd(uint8_t id, uint8_t val) {
    if (id == 1) {
        ...
        75 : val;
        if (rx_chan_global)
            iio_channel_attr_write_longlong(rx_chan_global, "hardwaregain", sys_rx_gain);
    } else if (id == 3) {
        sys_tx_atten = val > 80 ?
        80 : val;
        if (tx_chan_global)
            iio_channel_attr_write_longlong(tx_chan_global, "hardwaregain", sys_tx_atten);
    } else if (id == 4) {
        sys_tx_ampl = val * 128;
    }
...
void *telemetry_thread_func(void *arg) {
    ...
    while (keep_running) {
        ...
        if (tc && iio_channel_attr_read_longlong(tc, "input", &t_val) == 0)
            sys_radio_temp = t_val / 1000.0f;
        if (rx_chan_global && iio_channel_attr_read_longlong(rx_chan_global, "rssi", &r_val) == 0)
            sys_rssi = r_val / 100.0f;
        sleep(1);
    }
    return NULL;
}
        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            if (sample_count == 0 && bit_idx == 0) {
                ...
                if (uart_bridge_get_tx_packet(payload)) {
                    ...
                    // NY: UART ACK bakt inn, rå tidsstempel fjernet for å få plass
                    snprintf((char *)payload, PAYLOAD_LEN,
                             "L:%d|V:%d|K:%d|UA:%d|R:%.1f|C0:%02.0f|C1:%02.0f|T:%.1f",
                             last_cmd_id, last_cmd_val, cmd_ack_count, uart_ack_count, 
                             sys_rssi, sys_c0_load, sys_c1_load, sys_radio_temp);
                ...
                int temp_idx = 0;
                for (int b = 31; b >= 0; b--)
                    packet_bits[temp_idx++] = (current_sync >> b) & 0x01;
                for (int i = 0; i < PAYLOAD_LEN; i++)
                    for (int b = 7; b >= 0; b--)
                        packet_bits[temp_idx++] = (tx_payload[i] >> b) & 0x01;
            }
...
void *rx_thread_func(void *arg)
{
    ...
    symsync_crcf ss = symsync_crcf_create_rnyquist(
        LIQUID_FIRFILT_RRC,
        SAMPLES_PER_SYMBOL_RX,
        7,
        0.35f,
        32
    );
    ...
    while (keep_running) {
        if (iio_buffer_refill(rxbuf) < 0)
            continue;
        void *p0 = iio_buffer_first(rxbuf, rx0_i);
        void *pe = iio_buffer_end(rxbuf);
        ptrdiff_t step = iio_buffer_step(rxbuf);
        ptrdiff_t jump = step * DECIMATION_FACTOR;
        for (void *p = p0; p < pe; p += jump) {
            float re = ((int16_t *)p)[0] / 2048.0f;
            float im = ((int16_t *)p)[1] / 2048.0f;
            ...
            for (unsigned int i = 0; i < ns; i++) {
                ...
                if (st == 0) {
                    ...
                    if (nbit >= 24) {
                        uint8_t id  = (cmd >> 16) & 0xFF;
                        uint8_t val = (cmd >> 8)  & 0xFF;
                        uint8_t crc = cmd & 0xFF;
                        uint8_t t[2] = { id, val };
                    ...
                    if (nbit >= PAYLOAD_LEN * 8) {
                        ...
                        if (wr > 0) {
                            uart_ack_count++; // NY: Øker telleren når data når pinnen!
                            printf("\n[%s] UART <= RF %dB\n", ts, wr);
                        } else {
                            printf("\n[%s] UART pkt drop\n", ts);
                        }
...
int main() {
    ...
    if (uart_bridge_init("/dev/ttyPS1", 115200) != 0) {
        fprintf(stderr, "Kunne ikke åpne UART /dev/ttyPS1\n");
    }
    ...
    printf("🚀 Rakett-OS Kjører! (Sikker Data Whitening & UART ACK Aktivert)\n");
    fflush(stdout);

...
// --- DYNAMISKE SYSTEMVARIABLER (Nye for Link Optimization) ---
...
struct iio_channel *rx_chan_global; // For å styre RX Gain
struct iio_channel *tx_chan_global; // For å styre TX Attenuation
...
void *tx_thread_func(void *arg) {
    ...
    if (!txbuf) { printf("[FEIL] TX Buffer failed!\n"); return NULL; }
    ...
    while (keep_running) {
        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            ...
            if (sample_count == 0 && bit_idx == 0) {
                ...
                // NYTT: Inkluderer L(Id), V(Val), R(Rssi)
...
void *rx_thread_func(void *arg) {
    ...
    if (!rxbuf) { printf("[FEIL] RX Buffer failed!\n"); return NULL; }
    ...
    // State machine variabler
    int rx_state = 0; 
    int bits_read = 0;
    uint32_t cmd_payload = 0; 
    ...
    while (keep_running) {
        ...
        for (void *p = p_dat; p < p_end; p += fast_forward) {
            ...
            for (int i = 0; i < num_symbols; i++) {
                ...
                if (bit_count >= 50000) { 
                    get_ts(ts, sizeof(ts));
                    printf("[%s] [RX] Lytter på 433 MHz... (Radio Temp: %.1f°C | RSSI: %.1f)\n", ts, sys_radio_temp, sys_rssi);
                    fflush(stdout); bit_count = 0;
                }
                
                // NYTT: State Machine for å hente ut pakken
                if (rx_state == 0) {
                    shift_reg = (shift_reg << 1) | bit_out;
                    if (shift_reg == COMMAND_SYNC) {
                        rx_state = 1;      
                        bits_read = 0;     
                        cmd_payload = 0;   
                        shift_reg = 0;
                    ...
                    if (bits_read == 24) {
                        uint8_t id = (cmd_payload >> 16) & 0xFF;
                        uint8_t val = (cmd_payload >> 8) & 0xFF;
                        uint8_t crc = cmd_payload & 0xFF;
                        uint8_t chk[2] = {id, val};
                        
                        if (calculate_crc8(chk, 2) == crc) {
                            apply_cmd(id, val);
                            get_ts(ts, sizeof(ts));
                            printf("\n[%s] 🎯 KOMMANDO VERIFISERT (ID:%d, Verdi:%d)\n", ts, id, val);
                            fflush(stdout);
                        } else {
                            printf("\n[RX] Støy blokkert av CRC.\n");
                        }
                        rx_state = 0; 
                    }
                }
...
int main() {
    ...
    tx_chan_global = tx_chan; // Lagre til global peker
    ...
    iio_channel_enable(tx0_i); iio_channel_enable(tx0_q); // BEHOLDER BEGGE I OG Q!
    ...
    rx_chan_global = rx_chan; // Lagre til global peker
    ...
    iio_channel_enable(rx0_i); iio_channel_enable(rx0_q); // BEHOLDER BEGGE I OG Q!
    ...
    printf("🚀 Rakett-OS Kjører! (Stabilt minne + Full Link Control)\n");
    fflush(stdout);

...
#define AMPLITUDE 10000  // Redusert litt for å unngå matematisk "overshoot" fra filteret
...
void *tx_thread_func(void *arg) {
    ...
    int bit_idx = 0; 
    int sample_count = 0;
    float c0_load = 0.0, c1_load = 0.0;

    // --- MAGIEN: Pulsforming / Root Raised Cosine Filter ---
    // Dette filteret tar inn én og én bit, og spytter ut 50 avrundede, vakre samples
    ...
    while (keep_running) {
        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            ...
            // Hvis vi skal generere en NY bit
            if (sample_count == 0) {
                if (bit_idx == 0) {
                    memset(payload, 0x55, PAYLOAD_LEN);
                    get_core_loads(&c0_load, &c1_load);
                    
                    snprintf(payload, PAYLOAD_LEN, "A:%.1f|S:%d|K:%d|L:%d|C0:%02.0f|C1:%02.0f|T:%.1f|U:%.3f", 
                             rocket_alt, seq_num, cmd_ack_count, last_cmd_id, c0_load, c1_load, get_radio_temp(), get_raw_time());
                ...
                // Gir filteret den nye biten (+1.0 eller -1.0)
                float complex sym = packet_bits[bit_idx] ? 1.0f : -1.0f;
                // Filteret regner ut kurven og legger 50 glatte verdier inn i interp_buf
                firinterp_crcf_execute(interp, sym, interp_buf);
            }

            // Skriver den filtrerte, kurvede verdien til SDR-brikken i stedet for firkanten!
...
int main() {
    ...
    printf("🚀 Rakett-OS Kjører! Måler %% CPU per kjerne (C0=TX, C1=RX).\n");
    fflush(stdout);

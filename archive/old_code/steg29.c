...
// --- NYTT OG SKUDDSIKKERT: Henter temperatur direkte via IIO ---
struct iio_device *phy_dev_global; // Trenger denne globalt for å lese temp
float get_radio_temp() {
    if (!phy_dev_global) return 0.0;
    
    // Vi leter etter kanalen som heter "temp0" på AD9361-brikken
    ...
    // Leser verdien (som kommer i milligrader Celsius)
    ...
    while (keep_running) {
        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            if (bit_idx == 0 && sample_count == 0) {
                ...
                // Bruker nå get_radio_temp()
                snprintf(payload, PAYLOAD_LEN, "A:%.1f|S:%d|K:%d|L:%d|C:%.2f|T:%.1f|U:%.3f", 
                         rocket_alt, seq_num, cmd_ack_count, last_cmd_id, get_cpu_load(), get_radio_temp(), get_raw_time());
    ...
    while (keep_running) {
        ...
        for (void *p = p_dat; p < p_end; p += fast_forward) {
            ...
            for (int i = 0; i < num_symbols; i++) {
                ...
                if (bit_count >= 50000) { 
                    get_ts(ts, sizeof(ts));
                    printf("[%s] [RX] Lytter på 433 MHz... (CPU Load: %.2f | Radio Temp: %.1f°C)\n", 
                           ts, get_cpu_load(), get_radio_temp());
...
int main() {
    ...
    // Gjøre phy_dev tilgjengelig globalt slik at temperaturfunksjonen kan bruke den
    ...
    printf("🚀 Rakett-OS Kjører! Måler CPU & Temp via IIO.\n");
    fflush(stdout);

...
void *rx_thread_func(void *arg) {
    ...
    // --- SIKKERHETSBELTE 1: AGC Squelch ---
    agc_crcf qagc = agc_crcf_create();
    agc_crcf_set_bandwidth(qagc, 1e-3f);
    agc_crcf_squelch_enable(qagc);                // NYTT: Slår på støysperren
    agc_crcf_squelch_set_threshold(qagc, -40.0f); // NYTT: Fryser volumet hvis det blir for stille
    
    // --- SIKKERHETSBELTE 2: Symsync Bandwidth ---
    symsync_crcf qsync = symsync_crcf_create_rnyquist(LIQUID_FIRFILT_RRC, SAMPLES_PER_SYMBOL_RX, 7, 0.35f, 32);
    symsync_crcf_set_lf_bw(qsync, 0.02f); // NYTT: Holder timing-loopen stabil i brå overganger
            ...
            if (decim_count >= DECIMATION_FACTOR) {
                ...
                for (int i = 0; i < num_symbols; i++) {
                    ...
                    if (shift_reg == SYNC_WORD || shift_reg == SYNC_INVERTED) {
                        printf("\n=========================================\n");
                        printf("[RX] BINGO! KOMMANDO MOTTATT PÅ RAKETTEN!\n");
                        printf("=========================================\n\n");
                    }
                }
            }

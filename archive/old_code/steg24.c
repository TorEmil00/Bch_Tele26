...
// --- NØKLER ---
#define TELEMETRY_SYNC 0x1ACFFC1D // Nøkkelen raketten sender NED
#define SYNC_PING      0x1ACFFC1D // Nøkkel 1 OPP (Ping)
#define SYNC_DEPLOY    0x7E5A33B9 // Nøkkel 2 OPP (Fallskjerm)
#define SYNC_ABORT     0xC3F0A155 // Nøkkel 3 OPP (Abort)

struct iio_context *ctx;
struct iio_device *tx_dev, *rx_dev;
struct iio_channel *tx0_i, *tx0_q, *rx0_i, *rx0_q;

// --- DELT MINNE (POSTKASSE) ---
...
void *tx_thread_func(void *arg) {
    ...
    while (keep_running) {
        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            ...
            // Genererer ny pakke sømløst når forrige er ferdig
            if (bit_idx == 0 && sample_count == 0) {
                memset(payload, 0x55, PAYLOAD_LEN);
                
                // Lese fra postkassen og bygge telemetri
                snprintf(payload, PAYLOAD_LEN, "ID:1 | ALT:%.1fm | SEQ:%d | ACK:%d | LCMD:%d", 
                         rocket_alt, seq_num, cmd_ack_count, last_cmd_id);
...
void *rx_thread_func(void *arg) {
    ...
    while (keep_running) {
        ...
        for (void *p = p_dat; p < p_end; p += p_inc) {
            ...
            // Desimerer fra 2.5MSPS til 250kSPS
            if (decim_count >= DECIMATION_FACTOR) {
                ...
                // Krasj-barriere for å overleve støy-spikes
                ...
                for (int i = 0; i < num_symbols; i++) {
                    ...
                    // --- SJEKKER MOT DE TRE FORSKJELLIGE KOMMANDOENE ---
                    if (shift_reg == SYNC_PING || shift_reg == ~SYNC_PING) {
                        cmd_ack_count++; last_cmd_id = 1;
                        printf("\n[ROCKET] 🎯 BINGO! PING mottatt!\n");
                        fflush(stdout); shift_reg = 0;
                    } 
                    else if (shift_reg == SYNC_DEPLOY || shift_reg == ~SYNC_DEPLOY) {
                        cmd_ack_count++; last_cmd_id = 2;
                        printf("\n[ROCKET] 🪂 BINGO! FALLSKJERM-kommando mottatt!\n");
                        fflush(stdout); shift_reg = 0;
                    } 
                    else if (shift_reg == SYNC_ABORT || shift_reg == ~SYNC_ABORT) {
                        cmd_ack_count++; last_cmd_id = 3;
                        printf("\n[ROCKET] 🚨 BINGO! ABORT-kommando mottatt!\n");
                        fflush(stdout); shift_reg = 0;
                    }
                }
            }
...
int main() {
    ...
    // --- Start Tråder med CPU-tilknytning ---
